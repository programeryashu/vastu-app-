...
Reads  seeds/vastu_chintamani.json  (53 entries extracted from
...
Usage:
  python scripts/import_vastu_entries.py [--src seeds/vastu_chintamani.json]
"""
...
DEFAULT_SRC = ROOT / "seeds" / "vastu_chintamani.json"
...
def main() -> None:
    ...
    # The app dataset is GENERATED by build_app_dataset.py — regenerated entries
    # carry pipeline_fact_id and must never be re-imported (would duplicate facts).
    generated = [e for e in entries if e.get("pipeline_fact_id")]
    if generated and not any(not e.get("pipeline_fact_id") for e in entries):
        print(f"✓ all {len(generated)} entries are pipeline-generated "
              f"(pipeline_fact_id present) — nothing to import")
        return
    entries = [e for e in entries if not e.get("pipeline_fact_id")]
    ...
    with db.connect() as conn:
        db.init_db(conn)
        rec.extraction_status = "imported"  # facts come from the seed, not file extraction
    ...
    print(f"✓ imported {added} fact(s) from {len(entries)} app entry(ies)")
    if skipped:
